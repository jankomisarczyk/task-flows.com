...
class PDFFindController {
  ...
  #onFind(state) {
    ...
    console.log("this._dirtyMatch", this._dirtyMatch);
    this.#state = state;
  ...
  #calculateWords(words, pageIndex) {
    ...
    for (let i = 0; i < words.length; i++) {
      const newMatches = [];
      const newMatchesLength = [];
      const matcherResult = this.match(words[i], pageContent, pageIndex);
      matcherResult?.forEach(({ index, length }) => {
        const [matchPos, matchLen] = getOriginalIndex(diffs, index, length);
        if (matchLen) {
          newMatches.push(matchPos);
          newMatchesLength.push(matchLen);
        }
      });
      const { mergedMatches, mergedMatchesLength } = this.#mergeSort(
        matches,
        newMatches,
        matchesLength,
        newMatchesLength
      ...
      matchesLength.length = 0;
      if (mergedMatchesLength) {
        matchesLength.push(...mergedMatchesLength);
      }
    }
    this.#calc(pageIndex, matches);
  }
  #mergeSort(matches, newMatches, matchesLength, newMatchesLength) {
    let i = 0,
      j = 0,
      k = 0;
    const n1 = matches.length;
    const n2 = newMatches.length;
    const mergedMatches = [];
    const mergedMatchesLength = [];

    while (i < n1 && j < n2) {
      if (matches[i] < newMatches[j]) {
        mergedMatches[k] = matches[i];
        mergedMatchesLength[k] = matchesLength[i];
        i++;
      } else if (matches[i] === newMatches[j]) {
        if (matchesLength[i] < newMatchesLength[j]) {
          mergedMatches[k] = newMatches[j];
          mergedMatchesLength[k] = newMatchesLength[j];
        } else {
          mergedMatches[k] = matches[i];
          mergedMatchesLength[k] = matchesLength[i];
        }
    ...
    return { mergedMatches, mergedMatchesLength };
  }
  #calc(pageIndex, matches) {
  ...
  showSnackbar(message) {
    ...
    <svg version="1.0" id="katman_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
	 width="75" height="50" viewBox="0 0 1589 1092" xml:space="preserve">
<style type="text/css">
	.st0{fill:#8779CD;}
	.st1{fill:#4991E7;}
	.st2{fill:#8A78CB;}
...
<g>
    ...
    `;
    snackbar.innerHTML = `${svgImage}searched for: <strong>${message.toUpperCase()}</strong>`;
  ...
  #nextMatch() {
    ...
    // to poniżej robi NEXT albo previous
